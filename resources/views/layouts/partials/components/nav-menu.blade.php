...
<nav x-show="openNavMenu" x-cloak @click.away="openNavMenu = false" @keydown.escape.window="openNavMenu = false"
    ...
    class="fixed z-40 top-0 left-0 flex flex-col w-48 h-screen pt-28 pl-4 bg-main shadow-lg rounded-r-xl max-md:w-screen max-md:rounded-none max-md:text-center max-md:pl-0 xl:hidden">
    <a href="{{ route('menus.index', ['category' => 'dishes', 'viewType' => 'month']) }}"
        class="text-white text-xl font-bold mb-4">献立表</a>
